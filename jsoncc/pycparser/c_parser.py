...
class CParser(PLYParser):
    ...
    def _build_declarations(self, spec, decls, typedef_namespace=False, json=[]):
    ...
    def p_json_kwlist(self, p):
        """ json_kwlist : JSON_KEYWORD JSON_COLON json_kwlist 
                        | JSON_KEYWORD
        """
        p[0] = [p[1]] if len(p) == 2 else [p[1]] + p[3]

    def p_json(self, p):
        """ json : JSON_MODE JSON_COLON json_kwlist declaration
                 | JSON_MODE declaration
        """
        if len(p) == 5:
            p[0] = [c_ast.Json(p[1], p[3], p[4][0], self._coord(p.lineno(1)))]
        else:
            p[0] = [c_ast.Json(p[1], [], p[2][0], self._coord(p.lineno(1)))]
    ...
    def p_declaration_specifiers_3(self, p):
        ...
        """
        if len(p) > 3:
            p[0] = self._add_declaration_specifier(p[3], p[2], 'storage', p[1])
        else:
    ...
    def p_struct_or_union_specifier_1(self, p):
        ...
        if issubclass(klass, c_ast.Struct):
            p[0] = klass(
                name=p[2],
                decls=None,
                json=[],
    ...
    def p_struct_declaration_1(self, p):
        """ struct_declaration : specifier_qualifier_list struct_declarator_list_opt SEMI 
                               | specifier_qualifier_list struct_declarator_list_opt SEMI json_kwlist
        """
        json = []
        spec = p[1]
        assert 'typedef' not in spec['storage']
    ...
    def p_declarator_4(self, p):
        """ declarator  : declarator json_kwlist
        """
        p[0] = p[1]
